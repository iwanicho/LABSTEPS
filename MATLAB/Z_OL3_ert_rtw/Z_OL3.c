    ...
    if (1 == runModel) {
      {
        ...
        /* Output and update for function-call system: '<Root>/ADC-PWM Subsystem' */
        {
          ...
          /* '<S3>:1:10' */
          /* '<S3>:1:11' */
          tmp = floor((1.0 - (Z_OL3_P.VinNom - Z_OL3_P.IoutNom / (1.0 - (1.0 -
            Z_OL3_P.VinNom / Z_OL3_P.VoutNom)) * Z_OL3_P.RserL / 3.0) /
                       Z_OL3_P.VoutNom) * Z_OL3_P.TBPRD);
          ...
          rtb_CastToSingle2 = tmp < 0.0 ? (uint16_T)-(int16_T)(uint16_T)-tmp :
            (uint16_T)tmp;
